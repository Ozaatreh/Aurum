...
function convertPrice({ usdPerOunce, unit, currency, usdToJod }) {
...
async function evaluateSubscriptions() {
  ...
  for (const [email, subscription] of subscriptions.entries()) {
    ...
    const reentered = previousStatus === "within_range" || previousStatus === undefined;
    if (reentered && previousStatus !== status) {
      const breached = status === "below_range" ? "below lower threshold" : "above upper threshold";
      try {
...
app.post("/api/monitor", async (req, res) => {
  const { email, unit, currency, lower_threshold, upper_threshold } = req.body || {};
...
async function startMonitoring() {
  ...
  setInterval(refreshReferencePrices, 60 * 1000);
  setInterval(evaluateSubscriptions, 60 * 1000);
}
